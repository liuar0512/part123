...
class PartSegmentNetwork(nn.Module):
    def __init__(self, d_feature, d_in, d_out):
        super().__init__()

        W = d_feature

        self.partseg_lin_1 = nn.Linear(W, d_feature // 2)
        self.partseg_activation = nn.ReLU()
        self.partseg_lin_2 = nn.Linear(d_feature // 2, d_out)

        torch.nn.init.constant_(self.partseg_lin_1.bias, 0.0)
        torch.nn.init.normal_(self.partseg_lin_1.weight, 0.0, np.sqrt(2) / np.sqrt(d_feature // 2))

        torch.nn.init.constant_(self.partseg_lin_2.bias, 0.0)
        torch.nn.init.normal_(self.partseg_lin_2.weight, 0.0, np.sqrt(2) / np.sqrt(d_out))

    def forward(self, points, feature_vectors):
        x = feature_vectors
        x = self.partseg_lin_1(x)
        x = self.partseg_activation(x)
        x = self.partseg_lin_2(x)
        return x
